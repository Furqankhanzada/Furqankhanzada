# furqan.codes — the site in this repo

Personal site for Muhammad Furqan — a single static page, no framework, no build step.

> **Note:** this repo is also `Furqankhanzada/Furqankhanzada`, the GitHub profile repo.
> `README.md` on `main` is the profile README that renders on the GitHub profile page —
> leave it alone. Site documentation belongs here, in `SITE.md`.


## Serving it

GitHub Pages serves `main`'s root directly. Every file here is what ships; there is nothing
to compile and no `node_modules`.

- `CNAME` — custom domain (`furqan.codes`)
- `.nojekyll` — tells Pages to serve files as-is

Pages config: **Settings → Pages → Source: Deploy from a branch → `main` / `/ (root)`**.
...
branch. That was more machinery than a one-page site needs. The old build is still in git history,
and the `gh-pages` branch is kept as a rollback target.
